...
def get_visual_description(card: Card) -> str:
    ...
    segments.append(f"in {card.style.environment}.")
...
def get_subject_description(card: Card):
    ...
    # subject = card.style.subject + "::2.5"
    # subject_section.append(subject)
    subject_section.append("cyberpunk::2")
...
def generate_desc(card: Card) -> str:
    ...
    if gpt_client().is_openai_enabled:
        ...
        prompt += f"Be creative about its results and abilities on the race track. "
        prompt += f" (do not use the word {card.style.subject.lower()} or {card.element.name.lower()} or the stats):\n"
        print(prompt)
